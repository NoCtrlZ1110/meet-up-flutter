...
class PlaceHistoryHelper {
  ...
  static final PlaceHistoryHelper instance = PlaceHistoryHelper._privateConstructor();
  ...
  static void _createDb(Database db, int newVersion) async {
    await db.execute('CREATE TABLE $tableName($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colName TEXT, '
  ...
  Future<int> updatePlace(WeMapPlace place) async {
    Database db = await this.database;
    int result = await db.update(tableName, place.toMap(), where: '$colId = ?', whereArgs: [place.placeId]);
  ...
  Future<int> deletePlace(WeMapPlace place) async {
    Database db = await this.database;
    int result = await db.delete(tableName, where: '$colId = ?', whereArgs: [place.placeId]);
  ...
  Future<bool> existPlace(int placeID) async {
    Database db = await this.database;
    List<Map<String, dynamic>> result = await db.query(tableName, where: '$colId = ?', whereArgs: [placeID]);
  ...
  Future<List<WeMapPlace>> getPlaceList({int? limit, int? day, int? month, int? year, bool? before}) async {
    Database db = await this.database;
    ...
    List<WeMapPlace> placeList = <WeMapPlace>[];
